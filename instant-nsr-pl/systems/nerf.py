...
import os
from systems.loss_utils import ssim as ca_ssim
...
class NeRFSystem(BaseSystem):
    ...
    def test_step(self, batch, batch_idx):  
        out = self(batch)
        W, H = self.dataset.img_wh
        render_img = out['comp_rgb'].view(H, W, 3).permute(2, 0, 1).unsqueeze(0).to(batch['rgb'])
        target_img = batch['rgb'].view(H, W, 3).permute(2, 0, 1).unsqueeze(0)
        psnr = self.criterions['psnr'](out['comp_rgb'].to(batch['rgb']), batch['rgb'])
        with autocast(enabled=False):
            ssim = ca_ssim(render_img, target_img)
            lpips_ = lpips(render_img, target_img)
    ...
    def test_epoch_end(self, out):
        ...
        if self.trainer.is_global_zero:
            ...
            psnr = torch.mean(torch.stack([o['psnr'] for o in out_set.values()]))
            minpsnr = torch.min(torch.stack([o['psnr'] for o in out_set.values()]))
            maxpsnr = torch.max(torch.stack([o['psnr'] for o in out_set.values()]))
            ...
            text = f"{self.config.dataset.object}:instant-nsr-pl:{self.config.dataset.bsdf_name}: PSNR={psnr} [min={minpsnr} max={maxpsnr}]-SSIM={ssim}-lpips={lpips}"     
            os.makedirs(f'{self.config.render_save_dir}/{self.config.dataset.object}', exist_ok=True)   
            with open(os.path.join(f'../output', self.config.dataset.object, "instant-nsr-pl-wmask.txt"), "w") as file:
                file.write(text + '\n')
            self.export()
