...
class BlenderDatasetBase():
    def setup(self, config, split):
        ...
        self.focal_x = meta['fl_x'] # scaled focal length
        self.focal_y = meta['fl_y']

        # ray directions for all pixels, same for all images (same H, W, focal)
        self.directions = \
            get_ray_directions(self.w, self.h, self.focal_x, self.focal_y, self.w//2, self.h//2).to(self.rank) # (h, w, 3)           
        ...
        for i, frame in enumerate(meta['frames']):
            ...
            mask_path_ = frame['file_path'].split('/')[0] + '/mask/' + os.path.split(frame['file_path'])[-1]
            mask_path = os.path.join(self.config.root_dir, mask_path_)
...
@datasets.register('openmaterial-wmask')
