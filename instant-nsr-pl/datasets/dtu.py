...
class DTUDatasetBase():
    def setup(self, config, split):
        ...
        img_sample = cv2.imread(os.path.join(self.config.root_dir, 'images', '000.jpg'))
        ...
        mask_dir = os.path.join(f'../Mitsuba3_material-main/groundtruth/Pineapple', 'mask')
        self.has_mask = True
        ...
        for i in range(n_images):
            ...
            if self.split in ['train', 'val']:
                img_path = os.path.join(self.config.root_dir, 'images', f'{i:03d}.jpg')
                img = Image.open(img_path)
                img = img.resize(self.img_wh, Image.BICUBIC)
                img = TF.to_tensor(img).permute(1, 2, 0)[...,:3]

                mask_path = os.path.join(mask_dir, f'{i:03d}.jpg')
